...
# arrival_rates = [5, 3, 2.5, 2, 1.9, 1.8, 1.7, 1.6, 1.5, 1.4, 1.3, 1.2, 1.1, 1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
# arrival_rates = [1/3]
...
for arrival_rate in arrival_rates:
    ...
    for _ in range(1000):
        ...
        slotted_avg_wait_ns, slotted_avg_wait_ew, slotted_served_ns, slotted_served_ew = slotted_traffic_light.simulate_slot_based_traffic(
            arrival_times_ns, arrival_times_ew, simulation_time, total_slot_time, safety_time_same, safety_time_switch,
            max_starvation_time
        )
        # Simulate both directions
        slotted_avg_wait_ns_d, slotted_avg_wait_ew_d, slotted_served_ns_d, slotted_served_ew_d = slotted_traffic_light.simulate_slot_based_traffic(
            arrival_times_ns, arrival_times_ew, simulation_time, total_slot_time_small, safety_time_same, safety_time_switch,
            max_starvation_time_small
        )
        slotted_results_ns.append(slotted_avg_wait_ns)
        slotted_results_ew.append(slotted_avg_wait_ew)
        slotted_results_overall.append((slotted_avg_wait_ns * slotted_served_ns + slotted_avg_wait_ew * slotted_served_ew)/(slotted_served_ns + slotted_served_ew))
        slotted_results_with_delay.append((slotted_avg_wait_ns_d * slotted_served_ns_d + slotted_avg_wait_ew_d * slotted_served_ew_d) / (slotted_served_ns_d + slotted_served_ew_d))
...
# fig.add_trace(go.Scatter(x=arrival_rates, y=slotted_avg_waiting_ls, mode='lines+markers', name='Slot-based'))
# fig.update_layout(
...
# fig.add_trace(go.Scatter(x=arrival_rates, y=slotted_avg_waiting_ns, mode='lines+markers', name='NS'))
# fig.add_trace(go.Scatter(x=arrival_rates, y=slotted_avg_waiting_ew, mode='lines+markers', name='EW'))
# fig.update_layout(
